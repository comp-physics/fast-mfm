...
[startX,startY] = meshgrid(x1(1:N1),x2(1:N2));

figure()
l = streamslice(startX,startY,u1(1:N1,1:N2)',u2(1:N1,1:N2)',2);
axis tight
set(l,'LineWidth',1.5)
set(l,'Color',[0.72 0.26 0.06]);
set(gcf, 'color', 'w');    
set(gca, 'color', 'w');
% title('Streamlines')
...
%% Plot
figure()
pcolor(log10(abs(inv(L_bar))))
axis ij
% caxis([-2 2])
colorbar
shading flat
title('$\bar{\mathcal{L}}^{-1}$','Interpreter','Latex')
...
figure()
pcolor(log10(abs(D(2:N1+1,2:N1+1))))
shading flat
axis ij
colorbar
...
%% Write D matrix to file for pgfplots
fileID = fopen('Dmat.txt','w');
mymat = full(D);
for i = 2:N1
    for j = 2:N1
        fprintf(fileID,'%4i %4i %14.6f\n',i-1,j-1,log10(abs(mymat(i,j))));
    end
    fprintf(fileID,'%4s\n',' ');
end
fclose(fileID);

%% Write Lbar matrix to file for pgfplots
fileID = fopen('Lbarmat.txt','w');
mymat = full(L_bar);
for i = 2:N1
    for j = 2:N1
        fprintf(fileID,'%4i %4i %14.6f\n',i-1,j-1,log10(abs(mymat(i,j))));
...
%% Write Lbar matrix to file for pgfplots
fileID = fopen('LbarInvmat.txt','w');
mymat = full(inv(L_bar));
for i = 2:N1
    for j = 2:N1
        fprintf(fileID,'%4i %4i %14.6f\n',i-1,j-1,log10(abs(mymat(i,j))));
    end
...
%% Write slices
fileID = fopen('D-slice.txt','w');
mymat = full(D);
mymat = mymat(N1/2,2:N1-1);
for i = 1:length(mymat)
    fprintf(fileID,'%8.4f %24.17f\n',x1(i+1),abs(mymat(i)));
end
fclose(fileID);

fileID = fopen('Lbar-slice.txt','w');
mymat = full(L_bar);
mymat = mymat(N1/2,2:N1-1);
for i = 1:length(mymat)
    fprintf(fileID,'%8.4f %24.17f\n',x1(i+1),abs(mymat(i)));
...
fileID = fopen('LbarInv-slice.txt','w');
mymat = full(inv(L_bar));
mymat = mymat(N1/2,2:N1-1);
for i = 1:length(mymat)
    fprintf(fileID,'%8.4f %24.17f\n',x1(i+1),abs(mymat(i)));
...
matrices = {...
  ...
  '2000N-3Nvx-2Nvy-D_2_times_41_matvecs.csv', ...
