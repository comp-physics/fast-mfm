...
fn = strcat(int2str(N1),'N1-',int2str(N2),'N2-');
...
writematrix(x1  ,strcat(fn,'x1_cells.csv'))
writematrix(x1_f,strcat(fn,'x1_faces.csv'))
...
% writematrix(L,strcat(fn,'L.csv'))
[iii,jjj,vals]=find(L);
dlmwrite(strcat(fn,'L.dat'),[iii jjj vals],'delimiter', ' ','newline','pc');
...
writematrix(L_bar,strcat(fn,'Lbar.csv'))
